...
`frontend/src/styles.css` maps these tokens onto the shadcn CSS variables, so
every UI component inherits the look.
...
- `tokens.css` — the canonical token block (warm clay palette, radii, shadows). **The
  ...
  (mirrored into `frontend/src/styles.css`) instead.
...
`frontend/src/styles.css` translates the clay tokens into shadcn's semantic variables
...
`frontend/src/styles.css` together.
